...
### map stress gradients for ms
png(filename='./fig/fig_00_stressgradient.png', wid=6.5,
...
### map NCI and NCIS for ms
png(filename='./fig/fig_00_map_crossspecies_responses.png', wid=9.5,
    hei=6.5, uni='in', bg='transparent', res=700)
set_par_mercury(5)
cc <- ecole::colvec(p$stress, begin=0.1, end=0.95, alpha=0.80)
plot(p$lon, p$lat, pch=16, cex=0.4, xlab='', ylab='', xlim=c(-125,-115),
...
png(filename='./fig/fig_00_across_spp.png',
...
# plot_lm(p$stress,  p$plt_ses_pc1, cex=0.2, xlab=x3, ylab=y5, col=cc)
...
# plot_lm(p$stress,  p$plt_ses_pc2, cex=0.2, xlab=x3, ylab=y6, col=cc)
dev.off()

### WITHIN-SPECIES models: test INTERACTIONS (eventually discard, bc few signif)
p$INTXN <- p$cmdlog * p$firelog  # interaction term
n_top    <- 20 # take only top-20 most frequent spp
j_pred   <- c('cmdlog','firelog','INTXN','plt_nci','plt_ses_pc1','plt_ses_pc2')
j_resp   <- c('rgr','surv')
j_all    <- c(j_resp, j_pred)
j_allspp <- c('spp',j_all)
p_few    <- p[ , ..j_allspp] # keep only the response+predictor columns
m <- unlist(lapply(j_resp, function(j) {
  fmla <- formula(paste0(j, '~ cmdlog + firelog + INTXN + ',
                         'plt_nci + plt_ses_pc1 + plt_ses_pc2'))
  lapply(nm[1:n_top], function(i) {
    p_i <- p_few[p_few$spp==i,]       # species subset
    data.table::setDF(p_i)
    p_i <- data.frame(sapply(p_i[,j_all], scale)) # scale
    stats::lm(formula = fmla, data = p_i)
  })
}), recursive=F)
`get_sig` <- function(x) { summary(x)$coefficients[,'Pr(>|t|)'] }
`get_coef` <- function(x) { coefficients(x) }
(is_sig <- sapply(m, get_sig) < 0.05)
plot_heatmap(is_sig)
(src <- sapply(m, get_coef))
plot_heatmap(src)
rm(pfew,j_pred,j_resp,j_all,j_allspp,p_few,m,get_sig,get_coef,is_sig,src)
# CONCLUDE: few interactions were significant, therefore omit from final models


#####     #####     #####     #####     #####     #####     #####     #####
### final WITHIN-SPECIES models: - all predictors in one model
#####     #####     #####     #####     #####     #####     #####     #####
set.seed(88)
n_boot <- 99 # ! ! ! TIMEWARN ! ! !
n_top  <- 20 # take only top-20 most frequent spp
j      <- c('rgr','surv') # responses
j_pred <- c('cmdlog','firelog','plt_nci','plt_ses_pc1','plt_ses_pc2') # 'stress'
...
s$predictor <- plyr::mapvalues(s$predictor, j_pred,
                               c('CMD','FIRE','CROWDING',
                                 'Func neigh PC1','Func neigh PC2')) # 'STRESS'
...
set_par_mercury(1)
png(paste0('./fig/fig_04_effectsize.png'),
...
png(paste0('./fig/fig_05_megamodel_sensitivity.png'),
